...
Writes these and is the only thing that should ever edit any of them:
  app/build/icon.html               the application icon
  brand/synartesis-logo.svg         the logo, on its ground
  brand/synartesis-logo-white.svg   the same, for a pale page
  brand/synartesis-mark.svg         the glyph alone, filled

The window shows `app/renderer/logo.png`, which is shot from the first of
those. It used to draw the mark itself from coordinates generated here, which
is how there came to be three logos that nearly matched.
"""
import math
...
def solid() -> str:
    """
    The same glyph as one filled shape, for the sizes the fan cannot survive.

    At sixteen pixels five hundred hairlines are a smudge, and the answer used
    to be a different drawing altogether -- a dashed circle with a rotated gap,
    close enough to pass and not the same mark. This is the identical geometry:
    the band between the same two true circles, over the same sweep, filled
    rather than combed. One glyph, two weights.
    """
    ox1, oy1 = at(R_OUT, START)
    ox2, oy2 = at(R_OUT, END)
    ix2, iy2 = at(R_IN, END)
    ix1, iy1 = at(R_IN, START)
    # The sweep is 254 degrees and runs anticlockwise on screen, so the long
    # way round with the flags that say so.
    return (
        f"M{ox1:.1f} {oy1:.1f} "
        f"A{R_OUT:.1f} {R_OUT:.1f} 0 1 0 {ox2:.1f} {oy2:.1f} "
        f"L{ix2:.1f} {iy2:.1f} "
        f"A{R_IN:.1f} {R_IN:.1f} 0 1 1 {ix1:.1f} {iy1:.1f} Z"
    )
...
def mark_svg() -> str:
    """The glyph filled, one colour, for small sizes and for the site."""
    return f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {SIZE} {SIZE}" role="img" aria-label="Synartesis">
  <!-- GENERATED by app/build/make-logo.py. Edit that, not this. -->
...
def logo_svg(ground: str = "#5e1420", ink: str = "#f6e9e5") -> str:
    """
    The mark at full detail, framed, on its ground. The logo.

    Two grounds, one drawing. The dark one is the logo; the light one is the
    same artwork for the places a dark tile would sit on a pale page looking
    like something stuck there.
    """
    lines = "\n".join(
        f'    <line x1="{x1:.1f}" y1="{y1:.1f}" x2="{x2:.1f}" y2="{y2:.1f}" />'
        for x1, y1, x2, y2 in body()
  ...
  {frame_svg(ink, 5.0, 0.55)}
  <g stroke="{ink}" stroke-width="1.2" opacity="0.34" fill="none" stroke-linecap="round">
{hairs}
  </g>
  <g stroke="{ink}" stroke-width="1.7" opacity="0.55" fill="none" stroke-linecap="round">
{lines}
  </g>
  <polygon points="{arrowhead()}" fill="{ink}"/>
</svg>
'''


open("app/build/icon.html", "w").write(icon_html())
open("brand/synartesis-mark.svg", "w").write(mark_svg())
open("brand/synartesis-logo.svg", "w").write(logo_svg())
open("brand/synartesis-logo-white.svg", "w").write(logo_svg("#fffaf3", "#5e1420"))
print("icon.html and the brand SVGs written")
